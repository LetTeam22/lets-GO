  ...
  try {
    ...
    if (experiences.length && fromDate != "null" && toDate != "null") {
      ...
      experiences = experiences.filter((experience) => {
        let show = false;
        if (
          experience.booking.startDate <= toDate &&
          experience.booking.startDate >= fromDate
        )
          show = true;
        return show;
      });
...
async function experienceDetails(req, res, next) {
  ...
  const expDetails = await Booking.findOne({
    include: [
      { model: Experience, where: { bookingIdBooking: bookingIdBooking } },
    ],
  });
...
async function createExperience(req, res, next) {
  let { imgExperience, textExperience, bookingIdBooking, firstName, email } =
    req.body;
  if (!textExperience && !bookingIdBooking && !firstName)
...
async function createExperienceWithApiGPT(req, res, next) {
  ...
  try {
    ...
    const [summary, sentiment, language] = await Promise.all([
      getApiGPTresponse(summaryPrompt, 0, 200),
      getApiGPTresponse(sentimentPrompt, 0, 200),
      getApiGPTresponse(languagePrompt, 0, 200),
    ]);

    const translation = language === "Español."
        ? "N/A"
        : await getApiGPTresponse(getTranslationPrompt(textExperience), 0, 200);
