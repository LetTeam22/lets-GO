...
export const CardExperience = ({
  ...
  const handleLike = (e, mail) => {
    ...
    if (!isAuthenticated) {
      ...
      if (!alreadyLike) {
        if (user?.email)
          dispatch(
            addLikeToDb({ idExperience: idExperience, email: user?.email })
          );
        ...
        if (user?.email)
          dispatch(removeLikeFromDb({ idExperience, email: user.email }));
      }
    }
  };
  ...
  const handleDelete = (e) => {
    ...
    swal({
      ...
      if (value) {
        ...
        const newExperiences = allExperiences.map((e) =>
          e.idExperience === idExperience ? { ...e, status: "deleted" } : e
        );
  ...
  const handleSave = (e) => {
    ...
    const newExperiences = allExperiences.map((e) =>
      e.idExperience === idExperience ? { ...e, textExperience: input.text } : e
    );
    dispatch(updateExperiencesState(newExperiences));
    setInput({
      id: "",
          ...
          <>
            <>
              {experienceIsLike(idExperience) ? (
                <button
                  onClick={(e) => handleLike(e, email)}
                  className={s.likeBtn}
                >
                  <FaHeart size="1.5rem" color="#F9B621" />
                </button>
              ) : (
                <button
                  onClick={(e) => handleLike(e, email)}
                  className={s.likeBtn}
                >
                  <FaRegHeart size="1.5rem" color="#F9B621" />
                </button>
              )}
            </>
                ...
                <>
                  {idExperience === input.id ? (
                    <button
                      ...
                      className={s.iconBtn}
                    >
                      <BiSave size="1.5rem" color="#F9B621" />
                    </button>
                  ) : (
                    <>
                      <button
                        ...
                        className={s.iconBtn}
                      >
                        <BiEdit size="1.5rem" color="#F9B621" />
                      </button>
                      <button
                        onClick={(e) => handleDelete(e, email)}
                        className={s.iconBtn}
                      >
                        <BiTrash size="1.5rem" color="#F9B621" />
                      </button>
                    </>
                  )}
                </>
